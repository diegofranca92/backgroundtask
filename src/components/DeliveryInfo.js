...
import { View, StyleSheet, Text, FlatList, SectionList, Image, TouchableOpacity, TouchableHighlight, Platform, ScrollView, RefreshControl,  Alert, BackHandler } from 'react-native';
...
function DeliveryInfo({navigation, bo_online, onTpStatusChange, onFinilizeDelivery}) {
  ...
  async function loadUser() {
   ...
   function initilizeSoundNotification(){
    ...
    var whoosh = new Audio('notification_sound.mp3', Audio.MAIN_BUNDLE, (error) => {
      if (error) {
  ...
  async function checkNewDelivery(bo_online){
    if(parseInt(bo_online) === 1){
      try {
        const cd_entregador = await AsyncStorage.getItem('cd_entregador');
        const response = await Api.api.get('/entregador/novaEntrega?cd_entregador='+cd_entregador, {
          auth: { 
  ...
  async function recuseDelivery(cd_entrega){
    ...
    try {
      const cd_entregador = await AsyncStorage.getItem('cd_entregador');
      const response = await Api.api.put('/entregador/recusarEntrega', {
  ...
  async function acceptDelivery(cd_entrega, index){
    ...
    try {
      const cd_entregador = await AsyncStorage.getItem('cd_entregador');
      const response = await Api.api.put('/entregador/aceitaEntrega', {
        "cd_entrega": cd_entrega,
        "cd_entregador": cd_entregador,
        "bo_aceita_entrega": "1"
      },{
  ...
  async function effectCollected(cd_entrega){
    try {
      const cd_entregador = await AsyncStorage.getItem('cd_entregador');
      const response = await Api.api.put('/entregador/origem', {
        "cd_entrega": cd_entrega,
        "cd_entregador": cd_entregador
      },{
        auth: { 
  ...
  async function goOutToDestination(cd_entrega){
    const cd_entregador = await AsyncStorage.getItem('cd_entregador');
    try {
      const response = await Api.api.put('/entregador/saidadestino', {
        "cd_entrega": cd_entrega,
        "cd_entregador": cd_entregador
      },{
        auth: { 
  ...
  async function arriveDestination(cd_entrega){
    const cd_entregador = await AsyncStorage.getItem('cd_entregador');
    try {
      const response = await Api.api.put('/entregador/destino', {
        "cd_entrega": cd_entrega,
        "cd_entregador": cd_entregador
      },{
        auth: { 
  ...
  async function finilizeDelivery(cd_entrega){
    ...
    setLoading(true);
    try {
      const response = await Api.api.put('/entregador/fimentrega', {
        "cd_entrega": cd_entrega,
        "cd_entregador": cd_entregador
      },{
        auth: { 
      ...
      if(response.status === 200){
        ...
        //onFinilizeDelivery("Entrega " + cd_entrega + " finalizada com sucesso!");
        setLoading(false);
      }
    } catch (error) {
      showMessage({
  ...
  if(statusCadastro === "A"){
    ...
    return (
      <>
        ...
        <ScrollView style={Object.keys(delivery).length > 0 || Object.keys(newDelivery).length > 0 ? styles.container : styles.hidden} showsVerticalScrollIndicator={false}>
          <View style={Object.keys(delivery).length === 0 ? styles.hidden : styles.listDelivery}>
          <FlatList
                ...
                return(
                  <View key={item.cd_entrega} style={styles.listItem}>
                    ...
                    <View style={styles.row}>
                      <Ionicons name="ios-alert" size={22} style={styles.iconDesc}/>
                      <Text style={styles.desc}> Buscar na empresa: </Text>
                      <Text style={styles.desc}>{item.empresa}</Text>
                    </View>
                    <View style={(item.hasOwnProperty("endereco_completo") && item.endereco_completo !== null) ? styles.row : styles.hidden}>
                      <Ionicons name="ios-pin" size={24} style={styles.iconDesc}/>
                      <Text style={styles.desc}>{item.endereco_completo}</Text>
                    </View>
                    <View style={(item.hasOwnProperty("nr_telefone_empresa") && item.nr_telefone_empresa !== null) ? styles.row : styles.hidden}>
                      <Ionicons name="ios-call" size={22} style={styles.iconDesc}/>
                      <Text style={styles.desc}> {maskPhone(item.nr_telefone_empresa)}</Text>
                    </View>
                    ...
                    <View style={styles.row}>
                      <Ionicons name="md-git-compare" size={22} style={styles.iconDesc}/>
                      <Text style={styles.desc}>{item.sn_retorno === "N" ? 'Não volta' : 'Com volta'}</Text>
                    </View>
                    <View style={styles.row}>
                      <Ionicons name="ios-cash" size={24} style={styles.iconDesc}/>
                    ...
                    <View style={styles.hidden}>
                      <Ionicons name="ios-person" size={22} style={styles.iconDesc}/>
                      <Text style={styles.desc}> Responsável: Nome do Cliente</Text>
                    </View>
                      ...
                      <>
                        ...
                        <View style={styles.row}>
                          <View style={styles.viewObs}>
                            <Text style={styles.textObs}>{item.ds_observacao}</Text>
                          </View>
                        </View>
                      </>
                    : <></>}

                    <View style={item.ts_aceite_entrega && !item.ts_chegada_origem && !item.ts_saida_destino && !item.ts_chegada_destino && !item.ts_entrega_finalizada ? {} : styles.hidden}>
                      <TouchableOpacity onPress={() => openMaps(item.nr_latitude_origem, item.nr_longitude_origem, item.empresa)} style={[styles.button, styles.button2]}>
                        <Text style={styles.buttonText}> Abrir Maps </Text>
                      </TouchableOpacity>
                      <TouchableOpacity onPress={() => effectCollected(item.cd_entrega)} style={styles.button}>
                        <Text style={styles.buttonText}> Fiz a Coleta </Text>
                      </TouchableOpacity>
                    </View>   
                    <View style={item.ts_chegada_origem && !item.ts_saida_destino && !item.ts_chegada_destino && !item.ts_entrega_finalizada ? {} : styles.hidden}>
                      <TouchableOpacity onPress={() => goOutToDestination(item.cd_entrega)} style={styles.button}>
                        <Text style={styles.buttonText}> Sair para o Destino </Text>
                      </TouchableOpacity>
                    </View>  
                    <View style={item.ts_saida_destino && !item.ts_chegada_destino && !item.ts_entrega_finalizada ? {} : styles.hidden}>
                      <TouchableOpacity onPress={() => arriveDestination(item.cd_entrega)} style={styles.button}>
                        <Text style={styles.buttonText}> Cheguei ao Destino </Text>
                      </TouchableOpacity>
                    </View>   
                    <View style={item.ts_chegada_destino && !item.ts_entrega_finalizada && item.sn_retorno === "N" ? {} : styles.hidden}>
                      <TouchableOpacity onPress={() => finilizeDelivery(item.cd_entrega)} style={styles.button}>
                        <Text style={styles.buttonText}> Finalizar Entrega </Text>
                      </TouchableOpacity>
                    </View>   
                    <View style={item.ts_chegada_destino && !item.ts_entrega_finalizada && item.sn_retorno === "S" ? {} : styles.hidden}>
                      <Text style={styles.avisoNoData}> Retorne ao estabelecimento para finalizar a entrega! </Text>
                    </View>   
                  </View>
                )
              }}
          ...
          <View style={Object.keys(newDelivery).length === 0 ? styles.hidden : styles.listDeliverys}>
              ...
              {newDelivery.map((item, index) => {
                return(
                  <View key={item.cd_entrega} style={[styles.listItemMiniCard, styles.rowMiniCard]}>
                    ...
                    <View style={styles.rowMiniCard}>
                      <TouchableOpacity onPress={() => recuseDelivery(item.cd_entrega)} style={[styles.buttonMiniCard, styles.button2]}>
                        <Text style={styles.buttonText}> Recusar </Text>
                      </TouchableOpacity>
                      <TouchableOpacity onPress={() => acceptDelivery(item.cd_entrega, index)} style={styles.buttonMiniCard}>
        ...
        <View style={Object.keys(delivery).length === 0 && Object.keys(newDelivery).length === 0 ? styles.noData : styles.hidden}>
          {/*<Ionicons name="ios-document" size={120} color="#666" style={styles.icon}/>*/}
          <Image
            source={iconeDelivery}
            style={parseInt(active) === 1 ? styles.iconeDelivery : styles.iconeDeliveryOpacity}
          />
          {parseInt(active) === 1 ? 
            <Text style={styles.avisoNoData}> Ativo, aguardando ofertas...</Text> :
            <Text style={styles.avisoNoDataOpacity}> Ative seu status para receber ofertas!</Text> 
          }
        </View>  
      </>    
    )
    ...
    return(
      <>
        ...
        <View style={styles.noData}>
          ...
          <Text style={styles.avisoNoData}> Para continuar finalize seu cadastro!</Text>
          <TouchableOpacity onPress={() => navigation.navigate('PageUserDetail', {page: "page-user-data", title: 'Dados Cadastrais'})} style={styles.button}>
    ...
    return(
      <>
        ...
        <View style={styles.noData}>
          <Image
            source={iconeDelivery}
            style={styles.iconeDeliveryOpacity}
          />
          <Text style={styles.avisoNoDataOpacity}> Aguardando a conclusão do treinamento...</Text>
        </View>  
      </>    
    )
      ...
      <>
        ...
        <View style={styles.noData}>
          <Image
            ...
            style={styles.iconeDeliveryOpacity}
          />
          <Text style={styles.avisoNoDataOpacity}> Sua conta está suspensa...</Text>
        </View>  
      </>    
    )
  } else {
